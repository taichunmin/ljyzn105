...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, Vcl.ComCtrls, SWComApi;
...
begin
    if ComboBox1.ItemIndex >= 0 then
      if SWCom_OpenDevice(ComboBox1.Items[ComboBox1.ItemIndex] ,115200) then
      begin
          EditShow.Clear;
          EditShow.Lines.Add('Open Success');
          if SWCom_GetDeviceSystemInfo($FF, @(pucSystemInfo[0])) then
            begin
...
begin    //WriteTag
    ...
    if SWCom_WriteCardG2($FF,@(Password[0]),Mem,WordPtr,Writelen,@(Writedata[0])) then
        begin
            ShowList('Success');
        end
    else
...
procedure TForm1.Button2Click(Sender: TObject);
  //ActiveCallback:FUNPTR_ACTIVE_CALLBACK;
begin
    ShowList('ActiveMode');
    SWCom_SetCallback(ActiveCallback);
...
begin
    ...
    if SWCom_InventoryG2($FF,@(pBuffer[0]),@(Totallen[0]),@(CardNum[0])) then
        begin
...
begin
    ...
    DevParamAddr := 5;
	  ...
	  if SWCom_ReadDeviceOneParam($FF,DevParamAddr,@(Value[0])) then
        begin
...
begin
    ...
    Value := Ord(ComboBox2.ItemIndex);
	  if SWCom_SetDeviceOneParam($FF,DevParamAddr,Value) then
        begin
            ShowList('Success');
        end
    else
