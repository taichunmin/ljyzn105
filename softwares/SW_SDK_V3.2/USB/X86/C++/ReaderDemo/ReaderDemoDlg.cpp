...
void CReaderDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_COMBO1, m_ctlUSB);
...
BEGIN_MESSAGE_MAP(CReaderDemoDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON30, &CReaderDemoDlg::OnBnClickedButton30)
...
void CReaderDemoDlg::OnBnClickedButtonOpen()
{
	...
	if(((CComboBox*)GetDlgItem(IDC_COMBO1))->GetCurSel() == -1) return;
	if(SWHid_OpenDevice(((CComboBox*)GetDlgItem(IDC_COMBO1))->GetCurSel()) == FALSE)
	{
		ShowString("Failed!\r\n");
		return;
	}
...
void CReaderDemoDlg::OnBnClickedButton10()
{
	...
	SWHid_SetCallback(Dev_Callback); //Callback with Json output
	ShowString("Success\r\n"); 
...
void CReaderDemoDlg::OnBnClickedButton30()
{
	char arrHidName[256];
	m_ctlUSB.ResetContent();
	int iHidNum = SWHid_GetUsbCount();
	for(int i=0; i<iHidNum; i++)
	{
		SWHid_GetUsbInfo(i, arrHidName);
		m_ctlUSB.AddString(arrHidName);
	}
	m_ctlUSB.SetCurSel(0);	
}
...
void CReaderDemoDlg::OnBnClickedButton14()
{
	...
	if (SWHid_WriteCardG2(0xFF,Password,Mem,WordPtr,WriteLen,Writedata) == false)
	{
		ShowString("Failed\r\n"); 
		return;
	}
...
void CReaderDemoDlg::OnBnClickedButton2()
{
	...
	if (SWHid_SetDeviceOneParam(0xFF,bParamAddr, bValue) == false)
	{
		ShowString("Failed\r\n"); 
		return;
	}
